...
function MediaPlayerRootImpl({
  ...
  const onMouseLeave = React.useCallback(
    ...
    [store, onMouseLeaveProp, autoHide, mediaPaused, menuOpen, dragging]
  );
  ...
  const onMouseMove = React.useCallback(
    ...
    [autoHide, onMouseMoveProp, onControlsShow]
  );
  ...
  const onKeyUp = React.useCallback(
    ...
    [onKeyUpProp, onVolumeIndicatorTrigger]
  );
  ...
  const RootPrimitive = useRender({
    ...
    props: mergeProps<"div">(
      // eslint-disable-next-line react-hooks/refs -- handlers/ref flow to the DOM element; refs are read only when events fire
      {
        ...
        onKeyUp,
        className: cn(
          "dark relative isolate flex flex-col overflow-hidden rounded-lg bg-black outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_video]:relative [&_video]:object-contain",
...
function MediaPlayerPlaybackSpeed({
  ...
  return (
    ...
    >
      ...
      <DropdownMenuPortal container={context.portalContainer}>
        <DropdownMenuContent
...
function MediaPlayerPortal({
  container: containerProp,
  children,
}: MediaPlayerPortalProps) {
